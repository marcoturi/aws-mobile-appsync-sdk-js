...
import { AUTH_TYPE, AuthOptions, USER_AGENT, USER_AGENT_HEADER } from '@marcoturi/aws-appsync-auth-link';
import { GraphQLError, print } from 'graphql';
import * as url from 'url';
import { v4 as uuid } from 'uuid';
import { CONTROL_MSG, MESSAGE_TYPES, ObserverQuery, SOCKET_STATUS, SUBSCRIPTION_STATUS, UrlInfo } from './types';
...
export class AppSyncRealTimeSubscriptionHandshakeLink extends ApolloLink {
	...
	private async _awsRealTimeHeaderBasedAuth({
		...
		const headerHandler: Record<string,
	...
	private async _awsRealTimeIAMHeader({
		...
		const result = await signatureV4.sign(request as any);
