 ...
 */
import { AUTH_TYPE, AuthLink, AuthOptions } from '@marcoturi/aws-appsync-auth-link';
import { createSubscriptionHandshakeLink } from '@marcoturi/aws-appsync-subscription-link';
import { ApolloCache } from 'apollo-cache';
import { ApolloReducerConfig, InMemoryCache, NormalizedCacheObject } from 'apollo-cache-inmemory';
import ApolloClient, {
	ApolloClientOptions,
	MutationOptions,
	MutationUpdaterFn,
...
import { ApolloLink, NextLink, Observable } from 'apollo-link';
...
import { defaultDataIdFromObject, OfflineCache } from './cache/index';
import { METADATA_KEY, OfflineCache as OfflineCacheType } from './cache/offline-cache';
import { boundEnqueueDeltaSync, buildSync, DELTASYNC_KEY, hashForOptions } from './deltaSync';
import { ComplexObjectLink, OfflineLink } from './link';
import ConflictResolutionLink from './link/conflict-resolution-link';
import { createRetryLink, PERMANENT_ERROR_KEY } from './link/retry-link';
import { createStore, DEFAULT_KEY_PREFIX, StoreOptions } from './store';
...
export const createAppSyncLink = ({
	...
	const link = ApolloLink.from([
		...
		createRetryLink(ApolloLink.from([
			new CatchErrorLink(() => new AuthLink({ url, region, auth }) as any),
			new PermanentErrorLink(createSubscriptionHandshakeLink({
				url,
				region,
				auth,
			} as any, resultsFetcherLink as any) as any),
		])),
	].filter(Boolean));
...
class AWSAppSyncClient<TCacheShape extends NormalizedCacheObject> extends ApolloClient<TCacheShape> {
	...
	constructor({
		...
		const link = waitForRehydrationLink.concat(customLink || createAppSyncLink({
			url,
			region,
			auth,
			complexObjectsCredentials,
		...
		this.hydratedPromise = disableOffline ? Promise.resolve(this) : new Promise(resolve => {
